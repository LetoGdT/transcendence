...
export class Ball implements Object2D
{
	...
	constructor(window: Window = {
		...
		initial_speed: number = 10, acceleration: number = 0.03, radius = 5)
	{
		this.window = window;
	...
	{
		if (new_position.x + this.radius > this.window.max.x || new_position.x - this.radius < this.window.min.x
			|| new_position.y + this.radius > this.window.max.y || new_position.y - this.radius < this.window.min.y)
			return true;
		return false;
	}
}